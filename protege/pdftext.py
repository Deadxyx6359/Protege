...
Protege never downloads anything, so pdfminer and PyMuPDF are not options. That
...
def _decode_stream(body: bytes) -> bytes | None:
    ...
    if b"LZWDecode" in filters:
        raise PdfError(
            "this PDF uses the LZW filter, which Protege cannot decompress. "
...
def extract_lines(raw: bytes) -> list[TextLine]:
    ...
    if not any(line.text.strip() for line in lines):
        raise PdfError(
            ...
            "are images, and reading them would need OCR, which Protege has no "
