...
Protégé was closed when a job was due, the next tick finds it overdue and does
...
from protege.core.agents import Kind, Trace
...
from protege.core.permissions.audit import redact
from protege.core.permissions.capabilities import ScopeKind, get
from protege.core.tools import ToolContext
            ...
            for job in sorted(self._jobs.values(), key=lambda j: j.next_run or 0.0):
                ...
                if not late:
                    ...
                    claimed.append((job, "skip",
                                    ...
                                    "Protégé was not running; skipped, as this job "
                                    "is set to do"))
                else:
                    claimed.append((job, "late",
                                    f"Missed {_plural(missed, 'scheduled run')} while "
                                    "Protégé was not running; ran once on return"))
        return claimed
