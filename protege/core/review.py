...
What it looks for, and why:
  ...
  * **Reaching for secrets** — any tool touching SSH keys, browser profiles,
    credential stores, or Protégé's own settings folder. The last is always
  ...
  * **Signs of tampering** — permissions for capabilities that do not exist in
    the grant file, or damage in the middle of the activity log. Protégé writes
...
from protege.core.permissions import CATALOGUE, AuditLog, Policy, SecretStore
from protege.core.permissions.audit import Event as AuditEvent
from protege.core.permissions.capabilities import Direction, Risk, ScopeKind, get
            ...
            for scope in grant.scopes:
                ...
                findings.append(Finding(
                    ...
                    + ("Anything an agent is steered into doing there cannot be "
                       "undone from inside Protégé." if writes else
...
def _file_findings(path: Path, now: float) -> list[Finding]:
    ...
    if unknown:
        findings.append(Finding(
            ...
            f"{', '.join(sorted(unknown))[:300]}. They grant nothing — Protégé "
            "ignores them — but Protégé never writes them either, so something "
    ...
    for event in events:
        ...
        for target in _targets(event):
            ...
            if normalised.startswith(guarded):
                code, severity = "protege-state", "critical"
            ...
            if key in seen:
                continue
            seen.add(key)
            if code == "protege-state":
                findings.append(Finding(
                    severity, code, "An agent reached for Protégé's own settings",
...
def _change_findings(events: list[AuditEvent], window_days: int) -> list[Finding]:
    ...
    return [Finding(
        ...
        "Check each was yours. Protégé only changes permissions when you do it in Settings.")]
...
def _secret_findings(secret_store) -> list[Finding]:
    ...
    return [Finding(
        ...
        "Nothing currently in Protégé uses: " + ", ".join(orphaned[:10])
...
def _log_findings(path: Path) -> list[Finding]:
    ...
    return [Finding(
        ...
        "means something other than Protégé changed the file.",
