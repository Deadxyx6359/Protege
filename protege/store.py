"""Atomic JSON persistence for Protege's config files.
...
class StoreError(RuntimeError):
    """Disk-level failure reading or writing a Protege file."""
...
def bootstrap_vault(vault: Path) -> tuple[Manifest, Settings, Personality]:
    ...
    try:
        ...
        raise SchemaError(
            f"{exc}\n\nProtege will not start with an unreadable configuration. "
