...
class SettingsWindow(tk.Toplevel):
    ...
    def _build_prompt_tab(self) -> None:
        ...
        self._note(
            tab,
            "Protege adds nothing to this but the lock directive, the unlocked-topic list, and your "
    ...
    def _build_security_tab(self) -> None:
        ...
        self._note(
            tab,
            "Tiers gate the model, not Protege. No tier grants network access -- that is enforced "
        ...
        tk.Label(
            ...
            text=f"Projects: {self.vault / 'projects'}   (switch projects from the status line; "
                 "point Protege at a different vault with --vault)",
    ...
    def _change_pin(self) -> None:
        if not self.manifest.pin.is_set:
            messagebox.showinfo("PIN", "No PIN is set yet. Restart Protege to set one.", parent=self)
            return
