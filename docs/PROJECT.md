...
| Live calls that survive the window being minimised, own widget | **Backend done**: hands-free, replies read as they stream in, a mute that closes the microphone; the call window is the interface's | D3 |
...
*Next:* nothing. The person chose not to have a wake word, which would keep the
microphone open whenever Akira runs; a call (D3) is the hands-free way in.
...
visible mute that actually stops capture; `core/voice/call.py`, bridge `Voice`
*Done, the backend:* the person chose calls without a wake word, so the
microphone is open only during a call they started. What they say is found by
its loudness against the room's (the quietest fifth of the last three
seconds), made out by Whisper, and sent to the chat; the reply is read aloud
as it streams in, a sentence at a time, which with a model writing two or three
words a second is the difference between hearing it at once and waiting
minutes. Muting closes the microphone rather than ignoring it. Withdrawing
Listen or Speak ends the call within a twentieth of a second. On speakers
Akira would hear itself, so by default it listens only while it is neither
thinking nor speaking, and the person interrupts with a button; with
headphones they can turn on interrupting by voice, which stops Akira and the
reply. The log keeps each call's length and how many things were said, never
what. A real round trip is tested: Kokoro's voice, played into a call, is
found, made out by Whisper and sent.
*Next (Codex):* the call's own always-on-top window, and keeping Akira running
while a call is on with the main window closed.
