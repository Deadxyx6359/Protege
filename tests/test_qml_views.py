...
Until these, nothing in the window used the Confirm or Permissions bridges: an
irreversible action waited five minutes for a dialog that did not exist and was
refused, and nothing could be granted at all. Each test runs the application's
own window offscreen, in its own process (it needs a QGuiApplication, and this
process may hold a QCoreApplication), with its settings in a temporary folder.
"""
...
def test_the_confirmation_and_the_permissions_work_in_the_window(run):
    out = run(PROBE)
