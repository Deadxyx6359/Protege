"""Data schemas for every file Akira persists.
...
failure mode of a corrupt config is "Akira refuses to talk", never "Akira
...
class Manifest:
    ...
    # Akira mode exact rather than a restore from memory.
...
class Settings:
    ...
    `system_prompt` is user-authored and starts empty. Akira never injects a
...
class Trait:
    ...
    `builtin` marks traits Akira ships. It affects nothing about behavior:
