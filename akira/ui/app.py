...
class AkiraWindow(tk.Tk):
    ...
    def _load_plugins(self) -> None:
        ...
        should be visible without being a modal obstacle to using Akira.
        """
    ...
    def _show_block_detail(self, key: str) -> None:
        ...
        if detail.layer == "tripwires":
            ...
            text.insert(
                "end",
                "\n\nA lock layer failed. Akira blocks rather than passing a response it could "
                "not fully check.",
            )
    ...
    def _effective_settings(self) -> Settings:
        ...
        the user's own material than Akira mode is.
        """
    ...
    def _on_send(self, text: str | None = None) -> None:
        ...
        if not status.library_available:
            messagebox.showerror(
                ...
                "llama-cpp-python is not installed, so Akira cannot run a model.\n\n"
        ...
        if not status.main_configured:
            messagebox.showerror(
                ...
                "Akira will not substitute a placeholder model.",
            )
            return
        ...
        try:
            while True:
                ...
                if kind == "token":
                    ...
                    # empty lines print under every "Akira" label.
    ...
    def _finish_turn(self, turn: Turn) -> None:
        if turn.blocked:
            ...
            self._transcript.append(f"AKIRA (blocked): {turn.text}")
        else:
            ...
            self._transcript.append(f"AKIRA: {cleaned or turn.text}")
    ...
    def _fail_turn(self, message: str) -> None:
        ...
        self._append(
            "No response was produced. Akira blocks rather than passing output it could not "
            "fully check.\n",
            "detail",
        )
        self._set_busy(False)
...
def run_app(vault: Path, manifest: Manifest, settings: Settings, personality: Personality) -> int:
    ...
    theme.set_app_id()
    try:
        window = AkiraWindow(vault, manifest, settings, personality)
    except tk.TclError as exc:
        print(f"Cannot start the user interface: {exc}")
        print("Akira needs a graphical display and a Tk-enabled Python build.")
        return 3
    ...
    window._append_system(
        f"Akira {__version__} -- {len(window.manifest.unlocked_topics)} topic(s) unlocked, "
