...
application. Akira itself always reads `vault/.protege/manifest.json` -- it has
to, that file is what defines the gate. Tier 0 means the model gets no
retrieval, no memory, and no note contents; it does not mean Akira cannot
...
@dataclass(frozen=True)
class PathPolicy:
    """Resolves and validates every path Akira touches.
    ...
    def app_roots(self) -> tuple[Path, ...]:
        """Roots Akira itself may touch, independent of tier.
    ...
    def resolve_app(self, candidate: str | os.PathLike[str]) -> Path:
        """Resolve a path Akira itself owns (manifest, settings, tripwires)."""
