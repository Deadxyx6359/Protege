...
first saves the previous version, in Akira's configuration folder rather
...
class Vault:
    ...
    def note_paths(self) -> Iterator[Path]:
        """Every note that may be read, skipping Obsidian's and Akira's folders."""
        count = 0
    ...
    def _writable(self, path: str | Path) -> Path:
        ...
        if any(part.startswith(".") or part in EXCLUDED_DIRS for part in parts):
            raise VaultError(f"{'/'.join(parts)} is inside a folder Obsidian or Akira keeps for itself.")
