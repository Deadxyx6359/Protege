"""The one way out: every request Akira makes to the network goes through here (C1).

Until this module, nothing in Akira could connect, and `verify_offline.py`
...
USER_AGENT = f"Akira/{__version__} (a personal assistant, fetching for its owner)"
...
def _resolve(host: str, port: int) -> list[str]:
    """The addresses \a host has: the only lookup Akira makes, let through the guard."""
    try:
